...
def _field_to_latlon_image(field: np.ndarray, cmap: str, vmin=None, vmax=None) -> Image.Image:
    """Render only the configured Antarctic AOI as a geographic PNG.

    The analysis grid is native EPSG:3031, while Leaflet ImageOverlay expects
    geographic WGS84 bounds.  Re-sample the valid grid cells into an AOI-sized
    regular lat/lon image.  Do NOT create a world-sized PNG or fabricate values
    outside the actual dataset: that makes the real Antarctic raster appear as
    a tiny rectangle near the bottom of a huge overlay.
    """
    g = global_grid

    # Keep the raster tightly aligned with the same bounds returned by the API.
    glat_min, glat_max = g.lat_min, g.lat_max
    glon_min, glon_max = g.lon_min, g.lon_max

    # High enough resolution for the existing 10-km analysis grid while keeping
    # the response small enough for a live API overlay.
    nlat, nlon = 300, 700
    img = np.full((nlat, nlon), np.nan, dtype=np.float64)

    # Convert the EPSG:3031 cell centres to AOI raster pixels.
    lat = np.asarray(g.lats)
    lon = np.asarray(g.lons)
    m = g.valid & np.isfinite(field)
    if np.any(m):
        rows = np.rint((glat_max - lat[m]) / (glat_max - glat_min) * (nlat - 1)).astype(int)
        cols = np.rint((lon[m] - glon_min) / (glon_max - glon_min) * (nlon - 1)).astype(int)
        rows = np.clip(rows, 0, nlat - 1)
        cols = np.clip(cols, 0, nlon - 1)
        img[rows, cols] = np.asarray(field, dtype=float)[m]

    # Fill small holes caused by re-sampling the projected grid, but ONLY from
    # actual AOI data. Never invent a global background field.
    finite = np.isfinite(img)
    if finite.any() and not finite.all():
        from scipy.ndimage import distance_transform_edt
        missing = ~finite
        _, nearest = distance_transform_edt(missing, return_distances=True, return_indices=True)
        # Only fill pixels close to real cells. This avoids smearing data across
        # the entire AOI when a source has genuine gaps.
        nearest_dist = distance_transform_edt(missing)
        fill = missing & (nearest_dist <= 4.0)
        if np.any(fill):
            ni, nj = nearest[:, fill]
            img[fill] = img[ni, nj]

    if vmin is None:
        vmin = float(np.nanpercentile(img, 2)) if np.isfinite(img).any() else 0.0
    if vmax is None:
        vmax = float(np.nanpercentile(img, 98)) if np.isfinite(img).any() else 1.0
