...
class RouteOptimizeRequest(BaseModel):
    ...
    # CUSTOM-mode weights (0-1 each; internally rescaled — see route_service.py
    # WEIGHT_SCALE constants — so a CUSTOM route with all weights at their
    # default sits close to BALANCED). Only used when mode == "CUSTOM"; the
    # FASTEST/SAFEST/BALANCED presets always use their own fixed weights so
    # existing behaviour for those modes is unaffected by these fields.
    distance_weight: float = Field(default=1.0, ge=0.0, le=5.0)
    safety_weight: float = Field(default=0.5, ge=0.0, le=1.0)
    fuel_weight: float = Field(default=0.3, ge=0.0, le=1.0)
    current_weight: float = Field(default=0.2, ge=0.0, le=1.0)

    # If set, only this mode is computed (in addition to always-on FASTEST/
    # SAFEST/BALANCED comparison set for backward compatibility, unless mode
    # is one of the three presets — see RouteService.optimize_route).
    ...
    def validate_mode(cls, v):
        if v is None:
            return v
        v = v.upper()
        if v not in ("FASTEST", "SAFEST", "BALANCED", "CUSTOM"):
            raise ValueError("mode must be one of FASTEST, SAFEST, BALANCED, CUSTOM")
        return v
...
class RouteResponse(BaseModel):
    ...
    # Added for fuller route metadata (Phase 6):
    route_score: Optional[float] = None
    warnings: List[str] = []
    data_availability: Dict[str, bool] = {}
    vessel_parameters: Optional[Dict[str, Any]] = None
